...
define([ "jquery", "collections/ActivityCollection", "views/ChannelView"], function( $, ActivityCollection, ChannelView ) {
	...
	var channelViews = new Array();
	
    return{    	
    	init: function(channelJsonData) {
    		$.each(channelJsonData, function (index, channel) {
    			var id = channel["code"];
    			var view = new ChannelView({ el: "#" + id, collection: new ActivityCollection( [] , { "channelCode": id, "channelId" : channel['uuid']
    			 , "channelName": channel['name'], "parentId": channel["parent_id"]} ) });
    			channelViews.push({"id" : id, "view" : view});
    		});
    	},
