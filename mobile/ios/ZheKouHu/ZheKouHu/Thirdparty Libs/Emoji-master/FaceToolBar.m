...
-(id)initWithFrame:(CGRect)frame superView:(UIView *)superView{
    ...
    if (self) {
        ...
        //可以自适应高度的文本输入框
        textView = [[UIExpandingTextView alloc] initWithFrame:CGRectMake(5, 5, 240, 36)];
...
//        [toolBar addSubview:voiceButton];
        ...
        faceButton.frame = CGRectMake(toolBar.bounds.size.width - 78.0f,toolBar.bounds.size.height-38.0f,buttonWh,buttonWh);
...
//        [sendButton setBackgroundImage:[UIImage imageNamed:@"send"] forState:UIControlStateNormal];
        ...
        sendButton.frame = CGRectMake(toolBar.bounds.size.width - 42.0f,toolBar.bounds.size.height-38.0f, buttonWh + 4,30);
...
-(void)inputKeyboardWillShow:(NSNotification *)notification{
    ...
    [UIView animateWithDuration:animationTime animations:^{
        ...
        CGFloat y = 0;
        CGRect superFrame = self.theSuperView.frame;
        ...
        if (currentOrientation == UIInterfaceOrientationPortrait || currentOrientation == UIInterfaceOrientationPortraitUpsideDown){//竖屏
            y = superFrame.size.height - keyBoardFrame.size.height;
        }else{
            y = superFrame.size.height - keyBoardFrame.size.width;
        }
        ...
        toolBar.frame = CGRectMake(0, y,  self.theSuperView.bounds.size.width,toolBar.frame.size.height);
    }];
    [faceButton setBackgroundImage:[UIImage imageNamed:@"face"] forState:UIControlStateNormal];
    keyboardIsShow=YES;
    [pageControl setHidden:YES];
