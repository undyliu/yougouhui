...
public class ShopProcessor extends ContentProcessor {
	...
	protected void updateContentProvider(RestMethodResult<Resource> result,
		...
		if(result.getResource() instanceof JSONObjResource){
			...
			String shopId = JSONUtils.getJSONStringValue(resource, DataConst.COL_NAME_UUID);
			try {
				JSONArray tradeList = resource.getJSONArray(ShopConst.NAME_REQUEST_PARAMETER_TRADES);//设置tradelist主营业务
				if(tradeList != null && tradeList.length() > 0){
					for(int i = 0; i < tradeList.length(); i++){
						JSONObject trade = tradeList.getJSONObject(i);
						JSONUtils.putJSONValue(trade, ShopTradeConst.COL_NAME_SHOP_ID, shopId);
						super.updateContentProvider(trade, ShopTradeData.COL_NAMES, ShopTradeConst.CONTENT_URI);
					}
				}
